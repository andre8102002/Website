...
<script src="https://unpkg.com/leaflet@1.3.4/dist/leaflet.js" integrity="sha512-nMMmRyTVoLYqjP9hrbed9S+FzjZHW5gY1TWCHA5ckwXZBadntCNs8kEqAWdrb9O7rxbCaA4lKTIWjDXZxflOcA=="
 ...
 <script src="assets/js/leaflet.ajax.js"></script>
   ...
   <script type="text/javascript">
    ...
    //marker
    var myIcon = L.icon({
    iconUrl: '<?=assets('icon/poin_gerak.gif')?>',
    iconSize: [30, 45],
 
});
    <?php 
	if($kategori!=''){
		$db->where('kategori','%'.$kategori.'%','LIKE');
	}
	if($tahun!='semua'){
    ...
    foreach ($ambil_data as $row) {
        ?>
        L.marker([<?=$row->latitude?>, <?=$row->longtitude?>],{icon: myIcon}).addTo(map).bindPopup("Kelurahan. <?=$row->nama_kel?><br>Tanggal : <?=$row->tanggal?><br>Ketinggian : <?=$row->ketinggian?><br>Kategori : <?=$row->kategori?><br>Lokasi : <?=$row->lokasi?><br>Latitude : <?=$row->latitude?><br>Longtitude : <?=$row->longtitude?>");
        <?php
    }
    ?>
