 ...
 <link rel="stylesheet" href="assets/js/Leaflet.markercluster-master/dist/MarkerCluster.css" />
   ...
   <script type="text/javascript">
    ...
    foreach ($ambil_data as $row) {
        ?>
        var marker = L.marker([<?=$row->latitude?>, <?=$row->longtitude?>],{icon: new myIcon({iconUrl: '<?=($row->marker=='')?assets('icon/red_marker.png'):assets('unggah/poin_marker/'.$row->marker)?>'})})
        .bindPopup("Kelurahan. <?=$row->nama_kel?><br>Tanggal : <?=$row->tanggal?><br>Ketinggian : <?=$row->ketinggian?><br>Kategori : <?=$row->kategori?><br>Lokasi : <?=$row->lokasi?><br>Latitude : <?=$row->latitude?><br>Longtitude : <?=$row->longtitude?>");
